...
#define GPIO_ANY_GPIO2                21
#define GPIO_OUT                17
// text below will be seen in 'cat /proc/interrupt' command
#define GPIO_ANY_GPIO_DESC           "sysfs"
...
static irqreturn_t r_irq_handler(int irq, void *dev_id, struct pt_regs *regs) {
   ...
   printk(KERN_NOTICE "Interrupt [%d] for device %s was triggered !.\n",
          irq, (char *) dev_id);
   gpio_direction_output(GPIO_OUT,true);
...
static irqreturn_t r_irq_handler2(int irq, void *dev_id, struct pt_regs *regs) {
   ...
   printk(KERN_NOTICE "Interrupt2 [%d] for device %s was triggered !.\n",
          irq, (char *) dev_id);
   gpio_direction_output(GPIO_OUT,false);
...
void r_int_config(void) {
   ...
   if (gpio_request(GPIO_ANY_GPIO, GPIO_ANY_GPIO_DESC)) {
      printk("GPIO request faiure: 25 \n");
      return;
   }

   if (gpio_request(GPIO_OUT, GPIO_ANY_GPIO_DESC)) {
      printk("GPIO request faiure: 17 \n");
      return;
   }
   if (gpio_request(GPIO_ANY_GPIO2, GPIO_ANY_GPIO_DESC)) {
      printk("GPIO request faiure: 21 \n");
      return;
   }
   ...
   if ( (irq_any_gpio2 = gpio_to_irq(GPIO_ANY_GPIO2)) < 0 ) {
      printk("GPIO to IRQ2 mapping faiure %s\n", GPIO_ANY_GPIO_DESC);
      return;
   }
   ...
   if (request_irq(irq_any_gpio2,
                   (irq_handler_t ) r_irq_handler2,
                   IRQF_TRIGGER_FALLING,
                   GPIO_ANY_GPIO_DESC,
                   GPIO_ANY_GPIO_DEVICE_DESC)) {
      printk("Irq Request2 failure\n");
      return;
   }
...
void r_int_release(void) {
   //gpio_direction_output(GPIO_ANY_GPIO,false);
   gpio_direction_output(GPIO_OUT,false);
   free_irq(irq_any_gpio, GPIO_ANY_GPIO_DEVICE_DESC);
   free_irq(irq_any_gpio2, GPIO_ANY_GPIO_DEVICE_DESC);
   gpio_free(GPIO_ANY_GPIO);
   gpio_free(GPIO_ANY_GPIO2);
   gpio_free(GPIO_OUT);
   return;
}
